...
namespace iGoo.Areas.Webcms.Models
{
    public class ReportViewModel : clsReport
    {
        ...
        public DataTable SelectByOrderCode(string sCodeOrder)
        {
            SqlCommand cmdToExecute = new SqlCommand();
            cmdToExecute.CommandText = "dbo.[sp_CMS_Orders_SelectByOrderCode]";
            cmdToExecute.CommandType = CommandType.StoredProcedure;
            ...
            try
            {
                cmdToExecute.Parameters.Add(new SqlParameter("@sOrderCode", SqlDbType.NVarChar, 200, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Proposed, sCodeOrder));
                if (_mainConnectionIsCreatedLocal)
                {
                    // Open connection.
                    _mainConnection.Open();
