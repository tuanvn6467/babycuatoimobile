...
{
    public class DTThongKeController : DefaultController
    {
        ...
        public ActionResult Index()
        {
            ...
            if(isFirstLoad == 1)
                list = iv.ReportDoanhThu_ThongKe().AsEnumerable().ToList();           
            ViewBag.BaoCao = list;
            isFirstLoad = 1;
        ...
        public void ExportData()
        {
            try
            {
                ...
                list = iv.ReportDoanhThu_ThongKe().AsEnumerable().ToList();
                var orderDetails = iv.ReportDoanhThu_ThongKeChiTiet().AsEnumerable();
                int stt = 0;
                using (var wb = new XLWorkbook(XLEventTracking.Disabled))
                {
                    //XLWorkbook wb = new XLWorkbook(XLEventTracking.Disabled);
                    string sheetName = "BaoCao_ThongKeDoanhThu"; //Give name for export file. 
                    var ws = wb.Worksheets.Add(sheetName);

                    # region header

                    ws.Cell(1,1).Value = "TT";
                    ws.Cell(1,2).Value = "Giờ hoàn tất";
                    ...
                    ws.Cell(1,12).Value = "NV bán hàng";
                    ws.Cell(1,13).Value = "Giao hàng";
                    ws.Cell(1,14).Value = "Loại KH";
                    ws.Cell(1,15).Value = "Chi nhánh";
                    ws.Cell(1,16).Value = "Ghi chú";
                    ...
                    for (int i = 0; i < list.Count; i++)
                    {
                        ...
                        ws.Cell(n, 12).Value = list[i]["nhanvien"];
                        ws.Range(n, 12, n + j - 1, 13).Column(1).Merge();
                        ws.Cell(n, 13).Value = list[i]["giaohang"];
                        ws.Range(n, 13, n + j - 1, 14).Column(1).Merge();
                        ws.Cell(n, 14).Value = list[i]["CusClassName"];
                        ws.Range(n, 14, n + j - 1, 15).Column(1).Merge();
                        ws.Cell(n, 15).Value = list[i]["InventoryName"];
                        ws.Range(n, 15, n + j - 1, 16).Column(1).Merge();
                        ws.Cell(n, 16).Value = list[i]["Comment"];
                        ws.Range(n, 16, n + j - 1, 17).Column(1).Merge();
                        n += j;
                    }
                    #endregion
                    ...
                    ws.Columns(7,12).Style.NumberFormat.Format = "#,###";
