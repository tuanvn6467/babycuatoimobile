...
{
    public class DTSanPhamController : DefaultController
    {
        private String per = Libs.GetPermission("ORDER");

        private List<DataRow> list;

        DTSanPhamViewModel iv = new DTSanPhamViewModel();
        ...
        public ActionResult Index()
        {
            ...
            list = iv.ReportDoanhThu_SanPham().AsEnumerable().ToList();            
            ViewBag.BaoCao = list;               
        ...
        public void ExportData()
        {
            try
            {
                ...
                list = iv.ReportDoanhThu_SanPham().AsEnumerable().ToList();
                List<BaoCaoDoanhThu_BySP_Export2> ReportList = new List<BaoCaoDoanhThu_BySP_Export2>();
                int stt = 0;
                for (int i = 0; i <= list.Count - 1; i++)
                {
                    stt++;                    
                    BaoCaoDoanhThu_BySP_Export2 id = new BaoCaoDoanhThu_BySP_Export2();
                    id.STT = stt.ToString();
                    id.MaSP = list[i]["FullName"].ToString();
                    id.ChungLoai = list[i]["ChungLoai"].ToString();
